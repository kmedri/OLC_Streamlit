...
st.markdown('conda install -c conda-forge pandas')
import pandas as pd
st.markdown('conda install -c conda-forge numpy')
import numpy as np
st.markdown('conda install -c conda-forge openpyxl')
import openpyxl
st.markdown('conda install -c conda-forge missingno')
import missingno as msno
import os
import io

st.markdown('data visualisation')
st.markdown('conda install -c conda-forge matplotlib')
import matplotlib.pyplot as plt
from matplotlib import rcParams
st.markdown('conda install -c conda-forge seaborn')
import seaborn as sns
st.markdown('conda install -c conda-forge folium')
...
st.markdown('apply some cool styling')
rcParams['figure.figsize'] = (12,  6)
sns.set(style='darkgrid', palette='pastel', font_scale=1)

st.markdown('For showing all columns in Pandas')
pd.set_option('display.max_columns', None)

st.markdown('this ignores the depreciation warnings etc')
import warnings
...
st.markdown('Now we can check datatypes.')
...
st.markdown('Lets take a sample row and have a look at each feature in more detail.')

st.dataframe(df_accident.sample(1))

st.markdown("Lets convert the categorical data to `category` and the numerical data to `int` and `float`.")
...
# st.bar_chart(data=df_avgs, x='year', y='yearly', alpha=0.6, ax=ax[0, 0])
# st.bar_chart(data=df_avgs, x='year', y='slight', alpha=0.6, ax=ax[0, 1])
# st.bar_chart(data=df_avgs, x='year', y='serious', alpha=0.6, ax=ax[1, 0])
# st.bar_chart(data=df_avgs, x='year', y='fatal', alpha=0.6, ax=ax[1, 1])
...
st.markdown("As expected we can see an overall decline in RTC's over the years, however in 2014 we see a rise in serious collisions.")
